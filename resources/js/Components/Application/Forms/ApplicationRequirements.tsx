...
const ApplicationRequirements = ({user, application, status, setApplication, setStatuses}: ApplicationFormProps) => {
    ...
    return (
        <Card className="sticky self-start top-0">
            <CardHeader className="flex-col items-start bg-success-300 mb-3">
            ...
            {/* Application Requirements for Status*/}
                            ...
                            <div className={`sm:grid grid-cols-3${index == requirementNames.length - 1 ? ' mb-2' : ''}`}>
                                ...
                                <div className="sm:mt-0 mt-4 col-span-2">
                                    ...
                                    {(uploaded.length > 0) ? uploaded.map((u, index) => {
                                        if (user.role === 'staff' && u.status.toLowerCase() === 'uploaded') {
                                            return (
                                                <p key={index} className="text-sm text-default-500">No files submitted</p>
                                            );
                                        }
...
const Footer = ({user, status, requirements, alert, loading, handleUpload, handleSubmit, handleApprove}: {
    ...
    else if (user.role === 'staff' && hasSomeSubmitted) {
        return (
            <>
                <Divider />
